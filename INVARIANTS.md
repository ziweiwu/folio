...
| I-2 | Layout measures display cells, never `String.length`. CJK and emoji are two cells, combining marks zero, and `U+FE0F` widens the glyph before it. A slice never separates a base glyph from its marks. Every scanner that accumulates width advances by *cluster* — a base with the marks bound to it — because measuring per code point is not the same as measuring the string: `⚠️` is two cells and its two code points are 1 and 0 | `core/width.ts` · `core/ansi.ts` · `test/layout.test.ts` · `test/ansi.test.ts` |
...
| I-6 | A resize re-anchors to the block that was at the top of the viewport, so the reader keeps their place instead of being thrown to the top — in the running app, not only in the arithmetic. The offset counts rows in the layout being replaced, so nothing may record a block from it until the re-anchor has committed | `core/position.ts` · `app.tsx` · `test/viewport.test.ts` · `test/app.test.tsx` |
...
| I-18 | A composed frame is exactly the terminal's width in both overflow modes, at any horizontal offset. Layout may emit rows wider than the screen in `scroll` mode; the viewport is what guarantees the frame — so decorating a row never shortens it, whatever width the decorator is handed. Clipping happens after, or a match near the start of a wide row blanks the part the reader panned to | `ui/chrome.ts` · `test/overflow.test.ts` |
...
| I-33 | Only the newest reload may change the document, and only while the reader is still on it. An editor that writes a file twice on one save fires two changes, and nothing orders their reads — so a reload that finishes after a later one has started is discarded rather than allowed to restore stale text under a "reloaded" note. A navigation invalidates the same way: a reload or a highlight build started on one document and resolving after a link was followed is discarded rather than allowed to replace the document now on screen. `reload` and `watch` follow the document being read, and the host unwinds what `open` advanced when the reader goes back | `app.tsx` · `cli.tsx` · `test/app.test.tsx` |
...
| I-23 | Copying is reported as *sent*, never as *copied*. OSC 52 is advisory — the terminal may silently refuse — so the message must not claim more than actually happened | `core/clipboard.ts` · `app.tsx` · `test/app.test.tsx` |

## CLI citizenship

| # | Invariant | Where |
|---|---|---|
| I-11 | Piped output carries no escape codes and no trailing whitespace. `NO_COLOR` is fully readable: headings keep their rules, tasks their boxes, code its frame. Colour is never the only encoding — including interactive state, which has no second channel of its own: a search match and the picked link are marked with reverse video when colour is off, an attribute rather than a colour, because otherwise they differ from the text around them by background alone and simply vanish | `md/layout.ts` · `core/ansi.ts` · `core/search.ts` · `test/layout.test.ts` · `test/viewport.test.ts` · `scripts/smoke.sh` |
